        ...
        switch (transType) {
            case TT_NEW_ORDER:
                if (useStoredProcedures) {
                    switch (dbType) {
                        case jTPCCConfig.DB_POSTGRES -> executeNewOrderStoredProcPostgres(log, db);
                        case jTPCCConfig.DB_ORACLE -> executeNewOrderStoredProcOracle(log, db);
            ...
            case TT_PAYMENT:
                if (useStoredProcedures) {
                    switch (dbType) {
                        case jTPCCConfig.DB_POSTGRES -> executePaymentStoredProcPostgres(log, db);
                        case jTPCCConfig.DB_ORACLE -> executePaymentStoredProcOracle(log, db);
            ...
            case TT_ORDER_STATUS:
                if (useStoredProcedures) {
                    switch (dbType) {
                        case jTPCCConfig.DB_POSTGRES -> executeOrderStatusStoredProcPostgres(log, db);
                        case jTPCCConfig.DB_ORACLE -> executeOrderStatusStoredProcOracle(log, db);
                        default -> throw new Exception("Stored Procedure for ORDER_STATUS not implemented");
                    }
                } else {
            ...
            case TT_STOCK_LEVEL:
                if (useStoredProcedures) {
                    switch (dbType) {
                        case jTPCCConfig.DB_POSTGRES -> executeStockLevelStoredProcPostgres(log, db);
                        case jTPCCConfig.DB_ORACLE -> executeStockLevelStoredProcOracle(log, db);
                        default -> throw new Exception("Stored Procedure for STOCK_LEVEL not implemented");
                    }
                } else {
            ...
            case TT_DELIVERY_BG:
                if (useStoredProcedures) {
                    switch (dbType) {
                        case jTPCCConfig.DB_POSTGRES -> executeDeliveryBGStoredProcPostgres(log, db);
                        case jTPCCConfig.DB_ORACLE -> executeDeliveryBGStoredProcOracle(log, db);
	...
	*/
    ...
    private void executeDeliveryBGStoredProcOracle(Logger log, jTPCCConnection db)
        ...
        try {
            ...
            System.arraycopy(delivery_array, 0, deliveryBG.delivered_o_id, 0, 10);
            db.commit();
            stmt.close();
        } catch (SQLException se) {
            log.error("Unexpected SQLException in DELIVERY_BG");
