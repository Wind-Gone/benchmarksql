...
public class jTPCCConnection {
    ...
    public jTPCCConnection(Connection dbConn, int dbType)
        ...
        switch (dbType) {
            ...
            case jTPCCConfig.DB_ORACLE -> stmtNewOrderStoredProcOracle =
        ...
        switch (dbType) {
            case jTPCCConfig.DB_POSTGRES -> stmtPaymentStoredProc = dbConn.prepareStatement(
                    "SELECT * FROM bmsql_proc_payment (?, ?, ?, ?, ?, ?, ?)");
            case jTPCCConfig.DB_ORACLE -> stmtPaymentStoredProcOracle =
        ...
        switch (dbType) {
            case jTPCCConfig.DB_POSTGRES -> stmtStockLevelStoredProc = dbConn.prepareStatement(
                    "SELECT * FROM bmsql_proc_stock_level (?, ?, ?)");
            case jTPCCConfig.DB_ORACLE -> stmtStockLevelStoredProcOracle =
        ...
        stmtDeliveryBGUpdateCustomer = dbConn.prepareStatement(
                "UPDATE bmsql_customer " +
                        ...
                        "    WHERE c_w_id = ? AND c_d_id = ? AND c_id = ?");
        switch (dbType) {
            case jTPCCConfig.DB_POSTGRES -> stmtDeliveryBGStoredProc = dbConn.prepareStatement(
                    "SELECT * FROM bmsql_proc_delivery_bg(?, ?, ?)");
            case jTPCCConfig.DB_ORACLE -> stmtDeliveryBGStoredProcOracle =
